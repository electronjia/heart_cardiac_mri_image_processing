...
marching_results_identifier = "_mask_marching_cubes"
...
new_dataframe_results = pd.DataFrame(columns=["patient_id", "dcm_image_filepath", "image_filepath", "mask_filepath",	"csv_filepath", kaiwen_results_column,  "dice", "iou"])
...
for idx, row in mask_filepaths_df.iterrows():
    ...
    if not pd.isna(mask_filepath):
        ...
        try:
            ...
            # predicted mask
            prediction_filepath = target_filepath.replace("_mask.png", kaiwen_results_identifier)
            ...
            new_dataframe_results.loc[len(new_dataframe_results)] = [ patient_id, dcm_img_filepath, img_filepath, mask_filepath, csv_filepath, dcm_img_filepath.replace(".dcm", kaiwen_results_identifier), None, None]

    else:
        new_dataframe_results.loc[len(new_dataframe_results)] = [ patient_id, 

      dcm_img_filepath, img_filepath, mask_filepath, csv_filepath, dcm_img_filepath.replace(".dcm", kaiwen_results_identifier), None, None
        ]


new_dataframe_results.to_excel(kaiwen_results_df_filepath, index=False)
